...
function WhatsAppButton() {
  ...
  useEffect(() => {
    const seen = localStorage.getItem("whatsapp_hint_seen");

    if (!seen) {
      setShowHint(true);
      localStorage.setItem("whatsapp_hint_seen", "true");
...
I came across your website and would like to know more about your event services 😊

Can you please share details and pricing?
  ...
  return (
    <>
      ...
      >
        ...
        {/* 🔥 BUTTON WITH PULSE */}
        <div className="relative bg-green-500 w-14 h-14 flex items-center justify-center rounded-full shadow-lg hover:scale-110 active:scale-95 transition duration-300 animate-pulseCustom">
          ...
          {/* 🔥 ONLINE DOT */}
          <span className="absolute bottom-1 right-1 w-3 h-3 bg-green-300 rounded-full border-2 border-white"></span>
      ...
      {showHint && (
        <div className="fixed bottom-24 right-4 md:right-5 bg-white text-black px-5 py-4 rounded-xl shadow-xl z-50 text-sm border w-[260px] animate-fadeIn">
          ...
          <p className="text-xs text-gray-600 mt-2 leading-relaxed">
            Chat with us instantly on WhatsApp for quick responses and event planning support.
          </p>
